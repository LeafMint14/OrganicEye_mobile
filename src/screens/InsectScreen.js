...
const InsectScreen = ({ navigation }) => {
    ...
    const fetchUserDataAndSubscribe = async () => {
      try {
        ...
        if (userResult.success && userResult.userData.pairedPiId) {
          ...
          // 2. The Fix: Add the Error Callback (second argument)
          unsubscribe = onSnapshot(
            ...
            (error) => {
              ...
              setLoading(false); // Stop the spinner so you aren't stuck!
              
              // If the error is about an index, alert the user
              if (error.message.includes("index")) {
                Alert.alert("Missing Index", "Check your Metro Console terminal. Click the link generated by Firebase to create the required index.");
              }
            }
          );
        } else {
  ...
  const renderItem = ({ item }) => {
    ...
    const isBeneficial = item.detection.toLowerCase().includes('beneficial') || 
                         item.detection.toLowerCase().includes('bee') || 
                         item.detection.toLowerCase().includes('ladybug');
      ...
      <TouchableOpacity 
        ...
        onPress={() => {
          if (isSelectMode) {
            ...
            // --- FIX 2: CLEAN DATA PASSING ---
            // Pass raw values. Do not wrap image in { uri: ... } here!
            navigation.navigate('InsectDetails', { 
              item: {
                id: item.id,
                name: item.detection, // Maps correctly for Details screen
                imageUrl: item.imageUrl, // Pass RAW string
      ...
      >
        <View style={styles.cardMain}>
          ...
          <View style={styles.cardInfo}>
            <Text style={[styles.cardStatus, { color: isBeneficial ? '#2D6A4F' : '#BC4749' }]}>
              {item.detection}
            </Text>
            <Text style={styles.cardTime}>
              {item.timestamp?.toDate ? item.timestamp.toDate().toLocaleDateString() : 'Just now'} • {item.confidence}% Confidence
            </Text>
          </View>

          <View style={styles.iconCircle}>
             <Ionicons 
                name={isBeneficial ? "shield-checkmark" : "warning"} 
                size={22} 
                color={isBeneficial ? "#2D6A4F" : "#BC4749"} 
             />
          </View>
        </View>
      </TouchableOpacity>
              ...
              ListEmptyComponent={
                <View style={styles.emptyContainer}>
                  ...
                  <Text style={styles.emptyText}>
                    {!pairedPiId ? "No IoT Device Paired" : "No Insects Detected Yet"}
                  </Text>
                  {/* Debug Helper */}
